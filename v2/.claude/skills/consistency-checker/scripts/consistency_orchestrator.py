...
def _head_basis_notice(root, diff_base):
    ...
    *default-branch* checkout (≈ ``diff_base``), NOT this task's worktree where
    ...
    spec together (e.g. a checker counts a constant's entries from the base
    checkout, sees fewer than HEAD, and asserts the newly-added entries are
    unimplemented). The orchestrator already runs *inside* the worktree
    (``root == os.getcwd()``), so it can hand the checker the one authoritative
    path and forbid missing-from-code conclusions drawn from the sub-agent's own
    CWD. A relative read is stale; an absolute read under ``root`` (or
    ``git -C root``) is HEAD-correct regardless of the CWD.
    """
    return (
        ...
        "checker 가 어떤 상수의 항목을 변경 전 개수로 세어 신규 항목 미구현이라 단언 → 정당한 PR BLOCK).\n"
