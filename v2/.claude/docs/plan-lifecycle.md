...
- spec-only PR 은 TEST WORKFLOW(unit)를 안 돌려 이 회귀가 그 PR 에서 안 잡히고 main 에 샌다 — `complete/` 이동 직후 최소 Gate C 가드(`spec-plan-completion.test.ts`)만이라도 프로젝트 unit 러너로 직접 돌려 확인.
