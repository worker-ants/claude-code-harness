...
This harness's workflow is carried by the role skills under `.claude/skills/`
(`developer`, `project-planner`, `consistency-checker`, `code-review-agents`,
`merge-coordinator`, `spec-coverage`).
...
risk firing on ordinary implementation prompts and competing with the
`developer` workflow. This file is an **example** of the "uninstall, keep a
pointer" pattern: on a given project, decide per skill whether the auto-trigger
overlap is worth the payoff, and record the rationale here. They were
